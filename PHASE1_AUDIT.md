...
| Time and cutoff contract | `utcnow()` and implicit timestamps were present in the runtime. | `SourceItem` now carries `SourceTimestamps`; normalized mappings record independent valid/missing/invalid states for published, updated, and retrieved timestamps; eligibility requires a valid publication timestamp inside the edition window. | Aware/naive, per-field malformed, missing/future, and cutoff tests pass. | `src/intelligence/contracts.py`, `src/intelligence/models.py`, `src/intelligence/production.py` |
...
- GitHub Actions run `34443774317` independently verified commit `bd680e7` on `main`: success, all jobs green, 21 seconds, after the workflow moved to `actions/checkout@v5` and `actions/setup-python@v6`.
